...
profiles = {
	...
	{
		name = "shortcut",
		function_name = "factor_and_speed_shortcut",
		metric = "time",
	}
}
...
-- the factor function for the shortcut profile
function factor_and_speed_shortcut (attributes, result)

	factor_and_speed (attributes, result)

	local shortcut = attributes.shortcut

	if shortcut then
		result.speed = (60 / 3.6) -- time = distance
		result.factor = 1
		result.canstop = false
		result.attributes_to_keep["shortcut"] = true
		return
	end
