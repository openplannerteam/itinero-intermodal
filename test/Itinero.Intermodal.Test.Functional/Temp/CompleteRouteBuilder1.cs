...
namespace Itinero.Intermodal.Test.Functional.Temp
{
    ...
    /// </summary>
    public class CompleteRouteBuilder1 : AlgorithmBase
    {
        ...
        /// </summary>
        public CompleteRouteBuilder1(RouterDb routerDb, Profile profile,
        ...
        private void Add(uint from, uint to, uint next, CancellationToken cancellationToken)
        {
            ...
            { // both are just regular vertices.
                ...
                if (edge == null)
                {
                    throw new Exception($"Could not build route, edge {from}->{to} not found.");
                }
                direction = !edge.DataInverted;
    ...
    public class Temp : IRouteBuilder
    {
